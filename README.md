...
2.任务4随时可以执行
因为可能是异步执行，无法确定执行完成时机，有时候需求可能需要我们在另一个地方执行这个任务完成后的逻辑，此时可以通过assignmentManager任务实例并通过assignmentManager.post(TaskThread4.class, () -> binding.sampleText.setText("張三李四王五"));
